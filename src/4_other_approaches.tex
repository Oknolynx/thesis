...
\todo{short notice about the history of VeraCrypt (TrueCrypt and other ancestors)}

\todo{\href{https://blog.quarkslab.com/resources/2016-10-17-audit-veracrypt/16-08-215-REP-VeraCrypt-sec-assessment.pdf}{VeraCrypt 1.18 Security Assessment}, \href{https://www.bsi.bund.de/SharedDocs/Downloads/EN/BSI/Publications/Studies/Veracrypt/Veracrypt.pdf?__blob=publicationFile&v=1}{BSI Security Evaluation of VeraCrypt}}
...
\todo{\texttt{InvalidateVolumeKeys}, \texttt{InvalidateDriveFilterKeys}, \texttt{crypto\_eraseKeys}, and the \texttt{burn} macro}

\todo{appendix on how to compile veracrypt in debug mode using VS 2019?}

\section{BitLocker}
\label{chap:otherapproaches.bitlocker}
\todo{better introduction, e.g. ``Microsoft has implemented... ships with more expensive editions of Windows...} In this section we describe the basic principles of Microsoft's BitLocker volume encryption. This will not be as detailed as \autoref{chap:background.luks2}. If more details are of interest, \cite{Kornblum2009} is a quite comprehensive reference. Note however that since its publication, some changes have been made to BitLocker: Windows 10 build 1511 introduced a new mode that uses the XTS block cipher mode, and the optional Elephant Diffuser was removed in Windows 8 \cite{Sosnowski2016}. Because the basics did not change, we still mostly rely on the findings presented in \cite{Kornblum2009}.

We will also look at \todo{some implementation} details by decompiling the source of the kernel driver that implements BitLocker's functionality.
%
%\todo{sources:}
%\begin{itemize}
%	\item \cite{Tuerpe2009}: UX in chapter 2.3, security considerations and attack strategies (mostly tpm specific but parts apply to all fde, maybe move to related work?), possible attacks \todo{(though all via TPM)}
%	\item \cite{Tan2020}: also describe the technical and cryptographic details plus a security analysis
%	\item \cite{Lewis2018}: not that relevant (more for related work), maybe still reference shortly? describes deployment of bitlocker and what lessons were learned
%\end{itemize}
...
Please refer to \autoref{chap:background.luks2.using} for the usage of an unlocked partition. Most of what is described there also applies to BitLocker.
...
\autoref{fig:otherapproaches.bitlocker.driverentry} shows that the BitLocker driver filters only IRPs with certain major functions, skipping all other requests. \todo{We employ a similar tactic for our driver.}
