...
Combining our observations from both access patterns, we draw the following \todo{conclusions}:
\begin{itemize}[beginpenalty=10000]
...
\paragraph{Discussion}
Combining our observations from all measurements, we draw the following \todo{conclusions}:
\begin{itemize}[beginpenalty=10000]
...
\todo{explain trends for other parameter values (smaller/bigger blocks), check how it looks for HDD?}: higher block size means more jagged/variation because the completion of one I/O request has more impact on the read rate the bigger it is (i.e. the boxes and whiskers have more height, less outliers)

\todo{looking at other block sizes, we could not find a general trend that one driver showed more or less variation in read rate than the others.}

\todo{we chose to display the graphs for 32 KiB blocks here because they seemed a good compromise between not being completely on one end of the spectrum of block sizes and not being to jagged.}
