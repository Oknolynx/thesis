...
Every WDM driver must implement a \texttt{DriverEntry} routine. It is called when the driver gets loaded and is responsible for driver initialization \cite{Wdk}.

\todo{\href{https://docs.microsoft.com/en-us/windows-hardware/drivers/kernel/driverentry-s-required-responsibilities}{this} and \href{https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nc-wdm-driver_initialize}{this}}

\todo{\texttt{DriverEntry} function, keep \autoref{fig:otherapproaches.bitlocker.driverentry} in mind}

\todo{generic dispatch: pass through everything if filtering not enabled, else either send IRP to specific dispatch routine or reject it}

\todo{add screenshot with WinDbg output for \texttt{!drvobj fvevol 7}? or maybe in \autoref{chap:otherapproaches.bitlocker.decompiling}}

\todo{briefly mention \texttt{DriverUnload} routine?}
