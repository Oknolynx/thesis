...
\cite{Fruhwirth2005} presents two templates for storing keys, TKS1 and TKS2. The difference is whether the key is encrypted before or after splitting it. LUKS and LUKS2 use TKS2, which is schematically explained in \autoref{fig:background.luks2.tks2}.
...
\begin{figure}[htb!]
	...
	\begin{tikzpicture}
		...
		\draw[arrow] (ciphr)                   -- node[anchor=south, yshift=-4pt] {\small\makecell{Splitted master key}} (merge);
	...
	\caption[
		...
		TKS2 scheme from \cite{Fruhwirth2005}.
	}
	\label{fig:background.luks2.tks2}
\end{figure}

\cite{Fruwirth2018}, Figure 5 provides pseudocode for deriving the master key. This process is identical for LUKS and LUKS2, although theoretically the parameters of the anti-forensic splitter or the digest algorithm could be changed in LUKS2 (in the JSON area). \autoref{fig:background.luks2.masterkeydecrypt} shows the outline of an implementation of the master key derivation in Rust.
