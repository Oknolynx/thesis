...
\begin{figure}[htb!]
	\center
	\begin{tikzpicture}
		\node[rect, fill=tYlow, from={0.2, 0.3 to 3.8, 7.1}] {\small Device node};
		...
		% \node[anchor=north] at (2, 0.1) {Device node};
...
\begin{figure}[htb!]
	...
	\small
	\begin{tikzpicture}
		\node[rectangle, fill=tPink,   from={0, 8 to 3, 9}] {\makecell{Major Function}};
		\node[rectangle, fill=tPink,   from={3, 8 to 6, 9}] {\makecell{Minor Function}};
		...
		\node[rectangle, fill=tYlow,   from={0, 3 to 6, 4}] {\makecell{FileObject}};
		\node[rectangle, fill=tYlow,   from={0, 2 to 6, 3}] {\makecell{DeviceObject}};
	...
	\caption[
		...
		Layout of the \texttt{IO\_STACK\_LOCATION} structure (modified after \cite{Yosifovich2017} using information from \cite{Wdk}). \todo{todo}
	}
	\label{fig:background.kerneldriver.directio}
\end{figure}
