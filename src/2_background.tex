...
\end{figure}
\footnotetext{\label{fn:background.kerneldriver.registryvolume} \texttt{HKLM\textbackslash SYSTEM\textbackslash CCS\textbackslash Control\textbackslash Class\textbackslash {71a27cdd-812a-11d0-bec7-08002be2092f}}, where \texttt{CCS} stands for \texttt{CurrentControlSet}. Later on in this section we will talk about the registry's role in driver management in more detail. \todo{ensure correct position}}

The device stack is built by the PnP manager during a process called \emph{device enumeration}. This happens at system boot or when resuming from hibernation, but it can also be triggered manually. During enumeration, the PnP manager constructs a so-called \emph{device tree} that captures the parent-child relationships of devices. A node in this tree represents a physical device and is appropriately called a \emph{device node}, or short \emph{devnode}. When a new device is discovered, its required drivers are loaded (if they have not already been loaded). The drivers are then informed about the new device \cite{Yosifovich2017}.
...
\begin{figure}[htb!]
	\center
	\todo{Insert screenshots here, use these keys:}
	\verb|HKLM\SYSTEM\CurrentControlSet\Control\Class\{71a27cdd-812a-11d0-bec7-08002be2092f}|
	\verb|HKLM\SYSTEM\CurrentControlSet\Enum\STORAGE\Volume\<insert instance id here>|
	\verb|HKLM\SYSTEM\CurrentControlSet\Services\fvevol|
	\caption[
		Example contents of a Hardware, Class, and Software subkey
	]{
		Example contents of a Hardware, Class, and Software subkey (Screenshots from the Windows Registry Editor). \todo{bla}
	}
...
\begin{itemize}
	...
	\item non-pageable paths
	\item memory
	\begin{itemize}
		\item accessing/locking user memory
		\item MDLs (``a structure that represents information for a buffer in physical memory.'', also page 530)
		\item non-paged vs paged pool (page 324)
	...
	\item INF files (mention https://docs.microsoft.com/en-us/windows-hardware/drivers/devtest/infverif)?
	\item DPCs, ALPCs (they're not relevant for our driver, I think)?
\end{itemize}
