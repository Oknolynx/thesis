...
The reference implementation\footnote{\label{fn:luks2referenceimpl} \url{https://gitlab.com/cryptsetup/cryptsetup}} is designed only for usage on Linux, which is why we developed a new Rust library for interacting with LUKS2 partitions. This is not a full equivalent, but only a cross-platform helper. Its task is to take care of all the cryptographic work needed before actually decrypting and encrypting data. Notably, it lacks the following features of the reference implementation:
\begin{itemize}[label=\textbf{--}]
	...
	\item actually mounting a LUKS2 partition for read/write usage (this is what our kernel driver is for).
\end{itemize}
...
\begin{figure}
	...
	\caption[
		...
		LUKS2 binary header example. The fields, as described in \autoref{fig:luks2binhdrstructure}, were coloured differently to be easily distinguishable. A similar header, although with different salt and hash, can be generated by executing \texttt{fallocate -l 16M luks2.img \&\& cryptsetup luksFormat ---label 'This is an ASCII label' ---subsystem 'This is an optional secondary label' ---uuid e93dcafa-ee0b-4168-aa7c-f30474886a2e luks2.img} in a Linux shell.
	}
	\label{fig:luks2binhdrexample}
\end{figure}

The sector containing the binary header is followed by the JSON area. This area arguably contains the metadata that is most relevant for decryption and encryption. \autoref{fig:luks2jsonobjects} contains an overview of the objects stored in JSON and their relationships. For this thesis' brevity's sake, please refer to Chapter 3.1 in \cite{Broz2018} for an example of a LUKS2 JSON area.
...
\cite{Fruwirth2018}, Figure 5 provides pseudocode for deriving the master key. This process is identical for LUKS and LUKS2, although theoretically other digest types than \texttt{pbkdf2} could be used in LUKS2.

\begin{lstlisting}[language=Rust, style=boxed]
fn decrypt_keyslot(/* ... */) -> Result<Vec<u8>, LuksError> {
