...
\todo{maybe \cite{Polasek2019} could be useful here?}
...
\todo{explain multiple keyslots and deriving the same master key using different passwords}
...
For a more detailed pseudocode for the master key derivation see \cite{Fruwirth2018}, Figure 5. \autoref{fig:background.luks2.masterkeydecrypt} shows the outline of an implementation of this process in Rust.
...
After a partition has been unlocked, i.e. after the master key of one of its segments has been decrypted, the partition is ready to be read from and written to. LUKS2 supports different segment encryption algorithms, see \autoref{fig:background.luks2.encryptionalgorithms} for a selection of them. Our driver only supports the default aes-xts-plain64 encryption. Therefore we will focus on that in this section.

The AES encryption algorithm is a block cipher for processing 128 bit data blocks \cite{Fips197}. To encrypt data longer than one block, a \emph{block cipher mode} is needed \cite{Ferguson2010}. These are encryption functions that build on a existing block cipher. When using aes-xts-plain64, LUKS2 uses the XTS block cipher mode \todo{[citation needed]}.
\todo{\cite{Ferguson2010} for explaining IVs}

\todo{plain64 means ``the initial vector is the 64-bit little-endian}\\\todo{version of the sector number, padded with zeros if necessary.'' \cite{Dmcrypt2020}} \\
\todo{This sector number is relative to the first sector of the segment, i.e. the first sector is}\\\todo{sector 0.}

\begin{figure}[htb!]
	\center
	\begin{tabular}{cc}
		\makecell{\textbf{Algorithm}\\in \texttt{dm-crypt} \cite{Dmcrypt2020} notation} & \textbf{Description} \\
		\hline
	...
	\caption[
		...
		Selection of LUKS2 encryption algorithms (modified after \cite{Broz2018}). \todo{See \cite{Dmcrypt2020} for a detailed explanation of the different elements of the algorithm notation.} \todo{cite/footnote for serpent/twofish/xts/cbc/essiv?}
	}
