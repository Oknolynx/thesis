...
One part of the OS that runs in kernel mode is the Windows executive, which was already mentioned in \autoref{fig:background.kerneldriver.winarchitecture}. It is responsible for multiple things \cite{Yosifovich2017}:
\begin{itemize}
	\item The implementations of the aforementioned system services reside in the executive.
	\item \texttt{DeviceIoControl}: not sure what the book means here, this just calls the NtDeviceIoControlFile syscall? \todo{look in VM with WinDbg what exactly happens here? screenshots?}\\\todo{if screenshots mention it when explaining syscalls above, may be a nice example}
...
Another major kernel mode OS component is the Windows kernel, also already presented in \autoref{fig:background.kerneldriver.winarchitecture}. Its tasks include the following \cite{Yosifovich2017}:
\begin{itemize}
...
\end{figure}
\footnotetext{\label{fn:background.kerneldriver.rdyboost} Which makes sense, because it is responsible for the ReadyBoost technology, which is designed for SD cards and USB sticks, and not hard disk volumes. \todo{ensure correct position}}

\subsubsection{Important Concepts for Kernel Driver Development}
\label{chap:background.kerneldriver.concepts}
The goal of this section is to explain some concepts that are relevant when developing a file system filter driver. They concern memory access, thread contexts, the kernel's I/O system, and communication between kernel and user mode. \todo{may need updating in the future}
...
In memory, the IRP is followed by one or more \emph{stack locations}, their count matching the number of drivers this IRP will pass through\footnote{\label{fn:background.kerneldriver.adjuststacksize} This value is known when allocating the IRP, because it is equal to the size of the device stack. In some scenarios, most of which involve the filter manager, an IRP might be sent over to a new device stack. This might necessitate readjusting the count of stack locations, if the new device stack has a different size than the one before \cite{Yosifovich2017}. We will discuss the filter manager in \autoref{chap:ourapproach.failed}.} \cite{Yosifovich2017}. These stack locations are represented by \texttt{IO\_STACK\_LOCATION} structures, whose layout is shown in \autoref{fig:background.kerneldriver.stacklocation}.

\begin{figure}[htb!]
	\center
	\small
	\begin{tikzpicture}
		\node[rectangle, fill=tPink,   from={0, 8 to 3, 9}] {\makecell{MajorFunction}};
		\node[rectangle, fill=tPink,   from={3, 8 to 6, 9}] {\makecell{MinorFunction}};
		\node[rectangle, fill=gray!30, from={0, 7 to 3, 8}] {\makecell{Flags}};
		\node[rectangle, fill=gray!30, from={3, 7 to 6, 8}] {\makecell{Control}};
		\node[rectangle, fill=tOrng,   from={0, 4 to 6, 7}] {\makecell{Parameters}};
		\node[rectangle, fill=tYlow,   from={0, 3 to 6, 4}] {\makecell{DeviceObject}};
		\node[rectangle, fill=tYlow,   from={0, 2 to 6, 3}] {\makecell{FileObject}};
		\node[rectangle, fill=tGren,   from={0, 1 to 6, 2}] {\makecell{CompletionRoutine}};
		\node[rectangle, fill=tLblu,   from={0, 0 to 6, 1}] {\makecell{Context}};

		\node[rect, fill=tBlue, from={7.5, 6.6 to 10.5, 7.4}] {\makecell{Read}};
		\node[rect, fill=tBlue, from={7.5, 5.6 to 10.5, 6.4}] {\makecell{Write}};
		\node[rect, fill=tBlue, from={7.5, 4.6 to 10.5, 5.4}] {\makecell{DeviceIoControl}};
		\node[rect, fill=tBlue, from={7.5, 3.6 to 10.5, 4.4}] {\makecell{$\ldots$}};
		...
		\draw[arrow] (6, 5.5) -> (7.5, 7);
		\draw[arrow] (6, 5.5) -> (7.5, 6);
		\draw[arrow] (6, 5.5) -> (7.5, 5);
		\draw[arrow] (6, 5.5) -> (7.5, 4);
	\end{tikzpicture}
	\caption[
...
\begin{figure}[htb!]
	...
	\begin{tikzpicture}
		...
		\draw[arrow] (irp.south) to [bend right=30] (0, 5.55);
	...
	\caption[
		...
		Example IRP flow (modified after \cite{Yosifovich2017}). \todo{todo}
	}
	\label{fig:background.kerneldriver.irpflow}
\end{figure}

``The split of information between the IRP body and its I/O stack location allows for the changing of I/O stack location parameters for the next device in the device stack, while keeping the original request parameters. [...] Also, note that completion routines can be registered by any layer (except the bottom-most one), each having its own place in an I/O stack location (the completion routine is stored in the next lower I/O stack location).'' \cite{Yosifovich2017}

\todo{all io is done on virtual files (page 486, 487)}

\todo{write about:}
\begin{itemize}
	\item IRPs (page 485, 513) and other IO stuff (page 485), Figure 6-13
	\item figure 6-3?
	\item figure 6-14? figure 6-18?
\end{itemize}

\todo{explain ``by the way'' when talking about \texttt{luks2flt}?}
\begin{itemize}
...
\todo{WinDbg (page 38), screenshots? don't take away content from \ref{chap:ourapproach.final.architecture}}