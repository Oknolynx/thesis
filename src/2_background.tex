...
\end{figure}
\footnotetext{\label{fn:background.kerneldriver.virtualmemory} Sven Peter has a nice quote on this topic on his \href{https://blog.svenpeter.dev/posts/m1_sprr_gxf/}{blog}: ``One of the kernel's tasks is to lie to each application running in userland and to tell them that they're the only one in the address space.''}
...
\begin{figure}[htb!]
	...
	\caption[
		...
		The executive takes care of managing memory, processes, and threads, as well as I/O, networking and other basic services. The kernel takes on the tasks of thread scheduling, dispatching interrupts and exceptions, and multiprocessor synchronization. Additionally, it provides routines and data structures for usage by other, higher-level components. Finally, the purpose of the Hardware Abstraction Layer (HAL) is to separate the kernel and device drivers from hardware-specific differences.
	}
...
\begin{itemize}
	\item Implementing system calls: the implementations aforementioned system services or syscalls reside in the executive.
	\item \texttt{DeviceIoControl}: \todo{look in WinDbg what exactly happens here? screenshots?}\\\todo{if screenshots mention it when explaining syscalls above}
	\item Support routines: the executive provides implementations of functions that can be used by other kernel mode components. Some of them are documented, others are not. The latter are e.g. functions used by the boot video driver.
	\item \todo{mention major components from page 73}
\end{itemize}

\todo{possible further content for this or later sections}
\begin{itemize}
	\item Kernel (page 75)
%	\item HAL? (page 79) (briefly, if at all)
