...
\subsection{Our LUKS2 Cryptographic Helper Library}
...
\begin{figure}[htb!]
	\begin{tikzpicture}[
		...
		\node[txt] at (2, 0.5) {\small\makecell{1\textsuperscript{st} JSON area}};
		...
		\node[txt] at (5.5, 0.5) {\small\makecell{2\textsuperscript{nd} JSON area}};
		\draw[fill=white] (7, 0) rectangle (10, 1);
		\node[txt] at (8.5, 0.5) {\small\makecell{Keyslots area}};
		...
		\node[txt] at (12.75, 0.5) {\small\makecell{Segment(s)}};
	\end{tikzpicture}
	\caption[
...
\begin{figure}[htb!]
	...
	\begin{tikzpicture}
		...
		\node[rect, anchor=west] (merge) at (8, 2) {\small AFmerge};
...
\begin{figure}[htb!]
	\center
	\begin{tikzpicture}
		\node[rect, fill=tPink,     from={4.25, 3   to 7.25, 5}]   {\small 128 TB\\System Space};
		\node[rect, fill={gray!30}, from={4.25, 2.1 to 7.25, 2.9}] {\small Unmapped};
		\node[rect, fill=tOrng,     from={4.25, 0   to 7.25, 2}]   {\small 128 TB\\User Process\\Space};
...
\begin{figure}[htb!]
	...
	\begin{tikzpicture}[
		...
		\node[rect, fill=tPink, from={0, 7.7   to 3.2, 8.3}] {\small System Processes};
		\node[rect, fill=tOrng, from={3.9, 7.7 to 7.1, 8.3}] {\small Service Processes};
		\node[rect, fill=tYlow, from={7.8, 7.7 to 11, 8.3}]  {\small User Processes};
		\node[rect, fill=tGren, from={1.6, 6.7 to 11, 7.3}]  {\small Subsystem DLLs};
		\node[rect, fill=tLblu, from={0, 5.7   to 11, 6.3}]  {\small Ntdll.dll};

		\node[anchor=south] at (12.5, 5.5) {\small User mode};
		\draw[dashed] (-0.25, 5.5) -- (13.75, 5.5);
...
\subsection{Windows Kernel Drivers and the Windows Driver Model}
...
As already hinted at in the description of \autoref{fig:background.kerneldriver.devicestack}, the registry is responsible for storing the information that governs driver loading. Which drivers are loaded and the order they are loaded in are both configured this way. Basically, it is described how exactly the device nodes should be built. \autoref{tbl:background.kerneldriver.registrykeys} lists the registry keys that, together with their subkeys, are relevant for the configuration. In \autoref{chap:ourapproach.final.install}, we will see how some of these registry entries are set up at driver installation.
...
\subsection{Important Concepts for Kernel Driver Development}
...
\paragraph{User Memory Access and I/O Modes}
...
\begin{figure}[htb!]
	...
	\small
	\begin{tikzpicture}
		\node[rectangle, fill=tPink,   from={0, 5.6 to 3, 6.4}]                    {MajorFunction};
		\node[rectangle, fill=tPink,   from={3, 5.6 to 6, 6.4}]                    {MinorFunction};
		\node[rectangle, fill=gray!30, from={0, 4.8 to 3, 5.6}]                    {Flags};
		\node[rectangle, fill=gray!30, from={3, 4.8 to 6, 5.6}]                    {Control};
		\node[rectangle, fill=tOrng,   from={0, 3.2 to 6, 4.8}]                    {Parameters};
		\node[rectangle, fill=tYlow,   from={0, 2.4 to 6, 3.2}, text depth=-0.5pt] {DeviceObject};
		\node[rectangle, fill=tYlow,   from={0, 1.6 to 6, 2.4}, text depth=-0.5pt] {FileObject};
		\node[rectangle, fill=tGren,   from={0, 0.8 to 6, 1.6}, text depth=-0.5pt] {CompletionRoutine};
		\node[rectangle, fill=tLblu,   from={0, 0   to 6, 0.8}]                    {Context};

		\node[rect, fill=tBlue, from={7.5, 4.6 to 10.5, 5.4}] {Read};
		\node[rect, fill=tBlue, from={7.5, 3.6 to 10.5, 4.4}] {Write};
		\node[rect, fill=tBlue, from={7.5, 2.6 to 10.5, 3.4}] {\ldots};
...
\begin{figure}[htb!]
	...
	\small
	\begin{tikzpicture}
		\node[rect, fill=gray!30, rounded corners=4pt, from={0, 7.2 to 4, 8.1}] (iomgr) {\small I/O Manager};
...
\begin{figure}[htb!]
	\center
	\todo{otherapproaches.veracrypt.devstack.png but for a volume with luks2flt attached?}
	% \includegraphics[scale=0.35]{../img/otherapproaches.veracrypt.devstack.png}
	\caption[
		WinDbg in remote kernel debugging mode
	]{
		WinDbg in remote kernel debugging mode. \todo{todo}
	}
